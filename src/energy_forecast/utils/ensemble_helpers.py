...
Provides ``build_context_features`` to extract temporal context
(hour, day_of_week, is_weekend, month, is_holiday) from a DatetimeIndex
into a target DataFrame.  Used by both ``models.ensemble`` (prediction)
and ``training.ensemble_stacking`` (meta-learner training).
"""
